...
exports.default = createDccpRoutes;
const express_1 = __importDefault(require("express"));
const DCCPBridge_1 = require("../core/DCCPBridge");
const compiler_1 = require("../core/compiler");
function createDccpRoutes(neuralRouter) {
    const router = express_1.default.Router();
    const bridge = new DCCPBridge_1.DCCPBridge();
    const compiler = new compiler_1.DCCPCompiler();
    /**
     * POST /api/dccp/route
    ...
    router.post('/batch', async (req, res) => {
        ...
        // 验证每个文件对象
        const invalidFiles = files.filter((f) => !f.filePath || f.content === undefined);
        if (invalidFiles.length > 0) {
            return res.status(400).json({
                error: 'DCCP-004',
                message: `Invalid files at indices: ${invalidFiles.map((f, i) => i).join(', ')}`
            });
        }
        try {
