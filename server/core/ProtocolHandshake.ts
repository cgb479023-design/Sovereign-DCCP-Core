...
import { DCCPPacket } from './compiler';
import { AgentConfig, AgentTier } from './AgentRegistry';
...
export class ProtocolHandshake {
  private static readonly tierCapabilities: Record<AgentTier, string[]> = {
  ...
  private static readonly forbiddenGenerationLimits: Record<AgentTier, string[]> = {
    'v1.5': ['AUTO_EVOLVE'],
    'v2.0': [],
    'vNext': []
  };
   ...
   */
  public static async verifyAlignment(
    packet: DCCPPacket,
    node: AgentConfig
  ...
  public static async verifyBatch(
    packet: DCCPPacket,
    nodes: AgentConfig[]
  ): Promise<Map<string, HandshakeResult>> {
  ...
  private static checkCapabilities(
    ...
    if (missingCapabilities.length > 0) {
      // 关键能力缺失
      const criticalMissing = missingCapabilities.filter(c => 
        ['json_mode', 'function_calling'].includes(c)
      );
  ...
  private static checkConstraints(
    ...
    // 检查 IPE 规则是否被节点支持
    const strictConstraints = packet.constraints.filter(c => 
      c.includes('STRICT') || c.includes('ZERO_PLACEHOLDER')
    );
   ...
   * 检查生成限制兼容性
   */
  private static checkGenerationLimit(
    packet: DCCPPacket,
    node: AgentConfig,
    warnings: string[],
    errors: string[]
   ...
   * 根据数据包确定所需能力
   */
  private static determineRequiredCapabilities(packet: DCCPPacket): string[] {
  ...
  private static logResult(
    packet: DCCPPacket,
    node: AgentConfig,
    result: { success: boolean; authorized: boolean; alignmentScore: number; recommendedAction: string }
  ): void {
