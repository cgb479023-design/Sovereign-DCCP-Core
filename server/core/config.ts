...
export interface DCCPConfig {
  ...
  bridge: {
    ...
    backupDir: string;
    backupMaxAge: number; // 天数
  };
}

const defaultConfig: DCCPConfig = {
  server: {
    port: 51124,
    env: 'development',
...
let config: DCCPConfig = { ...defaultConfig };
 ...
 * 获取配置
 */
export function getConfig(): DCCPConfig {
  return config;
}
    ...
    if (trimmed.startsWith('- ')) {
      if (!currentArray) {
        ...
        if (currentSection) {
          ...
          if (lastKey) currentSection[lastKey] = currentArray;
        }
      }
 ...
 * 创建默认配置文件
 */
export function createDefaultConfig(filePath: string = './dccp-config.yaml'): void {
